...
;;;            (setq should-be-in-entity (re-search-forward (concat " +" name "[ \n\t]+") nil t nil))	    
            (setq should-be-in-entity (re-search-forward (concat "[ \n\t]+" name) nil t nil))
			...
			(save-excursion (re-search-forward "\n" nil t nil)))
                ...
                (progn (backward-char) (setq found (point)))
	      )))
      (goto-char (point-min))
      ;;; TODO : state machine states can not be found
